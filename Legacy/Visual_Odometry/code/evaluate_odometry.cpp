...
vector<errors> calcSequenceErrors (vector<Matrix> &poses_gt,vector<Matrix> &poses_result) {
  ...
  // parameters
  int32_t step_size = /*10*/1; // every second
...
void plotPathPlot (string dir,vector<int32_t> &roi,int32_t idx) {
  ...
  for (int32_t i=0; i<2; i++) {
	...
	sprintf(command, "powershell.exe cd %s; gnuplot %s", dir.c_str(), file_name);
    system(command);
  }
  
  // create pdf and crop
  //sprintf(command,"powershell.exe cd %s; ps2pdf %02d.eps %02d_large.pdf",dir.c_str(),idx,idx);
  //system(command);
  //sprintf(command,"powershell.exe cd %s; pdfcrop %02d_large.pdf %02d.pdf",dir.c_str(),idx,idx);
  //system(command);
  //sprintf(command,"powershell.exe cd %s; rm %02d_large.pdf",dir.c_str(),idx);
  //system(command);
}
...
void plotErrorPlots (string dir,char* prefix) {
  ...
  for (int32_t i=0; i<4; i++) {
    ...
    // create pdf and crop
    //sprintf(command,"powershell.exe cd %s; ps2pdf %s_%s.eps %s_%s_large.pdf",dir.c_str(),prefix,suffix,prefix,suffix);
    //system(command);
    //sprintf(command,"powershell.exe cd %s; pdfcrop %s_%s_large.pdf %s_%s.pdf",dir.c_str(),prefix,suffix,prefix,suffix);
    //system(command);
    //sprintf(command,"powershell.exe cd %s; rm %s_%s_large.pdf",dir.c_str(),prefix,suffix);
    //system(command);
  }
}
...
bool eval (/*string result_sha,Mail* mail*/) {
  ...
  string result_dir     = "..\\poses\\results" /*+ result_sha*/;
